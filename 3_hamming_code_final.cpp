...
class hamming{
	public:
		...
		void setRedundantBits(){
			...
			int bit = 0;
			//outer loop runs for redundant bits (1 ,2 ,4 ,8 ,16 ,32 ....)
		...
		void receiver(){
			...
			if(ans.find('1') != string::npos){
				...
				for(int i = 0 ; i < ans.size() ; i++){
					...
					power*=2;// calculates redundant bit
				}
				cout << "bit number " << wrongbit << " is wrong and having error " << endl;
				msg[2] == '0' ? msg[2] = '1' : msg[2] = '0';
				cout<<" Bit Corrected ";
...
int main(){
	string data;
//	string data = "1011001";
	cout<<"Enter the data bits you wish to send: ";
	cin>>data;
	
	hamming h(data);
	
	// manipulating any ith data bit to check if receiver is detecting a error in that bit. 
	//If you eliminate the following line then correct code will be sent to receiver following that no error is received
	h.msg[6] == '0' ? h.msg[6] = '1' : h.msg[6] = '0';
